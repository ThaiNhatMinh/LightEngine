...
render::OpenGLPipeline::OpenGLPipeline(VertexShader * pVertexShader, PixelShader * pPixelShader)
{
	...
	static const char* attributeName[] = { "aPos", "aNormal", "aUV","aColor","aBlend1","aBlend2","aBlend3","aBlend4" };
...
void render::OpenGLPipeline::GetUniformList()
{
	...
	for (int i = 0; i < nUniform; i++)
	{
		...
		m_Uniforms.insert(std::make_pair( std::string(name),std::unique_ptr<PipelineParam>(DEBUG_NEW OpenGLPipelineParam(location,name))));
