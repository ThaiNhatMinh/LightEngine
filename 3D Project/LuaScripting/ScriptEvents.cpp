...
void ScriptEvent::RegisterEventTypeWithScript(const char* key, EventType type)
{
	...
	LuaPlus::LuaObject eventTypeTable = gLuaState()->GetGlobalVars().GetByName("EventType");
	if (eventTypeTable.IsNil())
		eventTypeTable = gLuaState()->GetGlobalVars().CreateTable("EventType");
...
ScriptEvent* ScriptEvent::CreateEventFromScript(EventType type)
{
	...
	{
		E_ERROR("Couldn't find event type");
		return NULL;
	}
}
...
{
	m_eventData.AssignNil(gLuaState()->GetLuaState());
}
